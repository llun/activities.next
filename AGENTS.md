...
`getNote` and `getActorPerson` fetch a URL and return whatever the body says; neither checks that the document's `id` is the id that was asked for. So a fetched `id` is a **claim by whoever answered the request**, exactly as much as its `content` is — and the moment that claim is used to resolve a database row, a remote server is choosing which of our rows a write lands on. Three sites got that wrong at once, and the sink was the same in each: `updatePoll`/`createAnnounce` resolve by a bare `where('id', ?)` with no ownership, locality or type filter.

- **Compare the fetched id against the id you REQUESTED, and normalize both with `normalizeActivityPubUri`.** `lib/services/polls/syncRemotePoll.ts` refuses a mismatch through its existing `failedPollSyncsAt` cooldown, then writes with `statusId: status.id` so the target is right by construction and not only by the guard. `lib/jobs/createAnnounceJob.ts` compares `boostedStatus.id` against the Announce's own `object` and drops the activity on a mismatch. Normalization is what keeps the guard from being over-strict: it collapses an explicit default port, percent-encoding and dot segments, and nothing else — a differing path, query or fragment is a different resource and stays refused. Host case never reaches either guard, because the HTTP client lowercases the authority before the request goes out.
