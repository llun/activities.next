...
- **`lib/database/statusVisibilityCallSites.test.ts` enforces this and is the reason the type is not simply required.** Every call to either method must name a visibility argument, spread an identifier named for one (`visibilityScope`, `audience`), or carry the `visibility-unfiltered` marker in a comment explaining why. A blanket "any spread counts" rule is too weak — `...(maxStatusId ? { maxStatusId } : null)` is a cursor — so the spread must be a plain identifier matching `/visibilit|audience|scope/i`.
