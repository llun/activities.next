...
- **A registration must write `oauthClient.clientCredentialsScopes`, and that is not the same column as `scopes`.** better-auth 1.6 validated a `client_credentials` request against the client's registered `scopes`; 1.7 moved the decision to a separate, server-owned column and denies the grant outright when it is missing or empty — `400 unauthorized_client` / `client has no authorized client_credentials scopes`. The column arrived with the 1.7 schema migration and nothing ever wrote it, so every application on the instance was refused an app token, and a native Mastodon client asks for one **before** it offers to sign a user in (Ivory does, with the credentials `POST /api/v1/apps` just handed it) — so the login never started, and the only sign of it was that 400 in the token proxy's log. `createApplication` writes the column through `toClientCredentialsScopes` (`lib/services/oauth/clientCredentialsScopes.ts`): the application's own registered scopes, minus the ones better-auth reserves for grants that delegate a user (`openid`, `profile`, `email`, `offline_access`). That restores what 1.6 granted and matches Mastodon, where an app token carries the scopes the application registered with — though it is **stricter than 1.6, not a replay of it**, and the rule is the useful form rather than a list of cases: the ceiling derives only from the client's OWN recorded scopes, so anywhere 1.6 reached past those, this does not. 1.6 reached past them in two ways — its rejection of the reserved scopes sat inside the same `if (requestedScopes)` conditional 1.7 uses, so a registered `openid` was granted verbatim when the client omitted `scope`; and `client.scopes ?? opts.scopes` fell back to the server's **entire** vocabulary for a client with no scopes recorded, which the backfill answers with `[]`. **That filter is the only thing enforcing it on this path** — better-auth's own assign-time validator runs inside its registration endpoints, which `createApplication` bypasses with a direct knex insert, and its token-endpoint filter runs only when the client sends an explicit `scope` parameter (with `scope` omitted the stored ceiling is granted verbatim), so do not delete the filter as redundant. An app token also lives **one hour**, not the 7 days `accessTokenExpiresIn` configures: `createUserTokens` reads `m2mAccessTokenExpiresIn` for a user-less grant and this server does not set it. Fixing the write path is **not sufficient on its own**: registrations are never deleted and clients cache their credentials indefinitely (see the bullet above), so every client already installed would stay wedged — `20260828000000_backfill_oauth_client_credentials_scopes` repairs the existing rows, and carries a **second copy** of the rule because a migration runs through the plain `knex` CLI with no TypeScript loader and no path aliases (`lib/database/sql/oauthClientCredentialsScopesMigration.test.ts` pins the two against each other). This grants no new authority: an app token has no user, so only `OAuthAppGuard` accepts one — `apps/verify_credentials` and Mastodon's API account registration, itself gated on `registrations.open` — and every other guard requires an actor that an actor-less token never resolves.
...
- **An account whose confirmation e-mail has not been clicked is refused with 403 on every authenticated surface — bearer and cookie alike — and `isActorConfirmationPending` (`lib/services/guards/OAuthGuard.ts`) is the check.** This is Mastodon's `require_user!` rule ("Your login is missing a confirmed e-mail address"), and it matters because `POST /api/v1/accounts` hands out a real 7-day user access token the moment an account is registered. `POST /api/v1/apps` is unauthenticated and its throttle only engages when `ACTIVITIES_TRUST_PROXY_IP_HEADERS` is set, so without this an anonymous party could script fully usable accounts — posting, uploading, following, federating — for addresses nobody has proven they control. The token is still issued, exactly as Mastodon issues it; what changes is that it is not a working credential until the address is confirmed.
- **Confirmation is read from `verificationCode`, NOT from `verifiedAt`, and that is not a style choice.** `accounts.verifiedAt` carries `DEFAULT CURRENT_TIMESTAMP` (`20230824181927_add_accounts_verification`), so `createAccount` could not leave it unset by omitting it: the database stamped `now()` on every pending registration, and `canCreateSessionForAccount`'s `verifiedAt` test has consequently **never fired**. A check keyed on `verifiedAt` is a no-op that reads as a working gate. `createAccount` now writes an explicit `verifiedAt: null` for a pending registration, so that column is accurate for anything created from here on — but rows written before that still carry the default, so `verificationCode` is the only signal covering both. It is set once at registration, cleared to `''` by `verifyAccount`, and never set at all on an instance with no e-mail configured.
- **The credential sign-in path was never open, which is why this is a token-path fix rather than an instance-wide one.** better-auth's own `emailAndPassword.requireEmailVerification` reads `accounts.emailVerified` — a different column, which `createAccount` correctly leaves false — and answers `403 EMAIL_NOT_VERIFIED`. So the web sign-up form has always produced accounts that cannot sign in; only the Mastodon API path produced live ones. Do not "simplify" the two signals together: `emailVerified` is not on the domain `Account`, and the guard needs a column the loaded actor already carries.
- **`allowUnconfirmedAccount` is the one carve-out, and it exists for exactly one endpoint.** `POST /api/v1/emails/confirmations` resends an account's own confirmation e-mail, so refusing it for being unconfirmed makes the state unrecoverable for a client that lost the message — Mastodon carves out the same controller (`Api::V1::Emails::ConfirmationsController` never calls `require_user!`). It relaxes the confirmation test and **nothing else**: `isActorModerationBlocked` still runs, so a suspended actor or a disabled account is refused there too, and the handler still requires a pending `verificationCode`. Do not add a second consumer without the same argument.
...
- **Existing accounts are deliberately grandfathered.** Nothing backfills `verifiedAt`, so an account that registered before this shipped keeps its defaulted value and is unaffected on both paths. Only the `verificationCode` half reaches such an account, and the only tokens it can withdraw are ones minted through the hole being closed — `registerViaApi` is the sole path that mints a user token for an unconfirmed account, since credential sign-in is refused and the authorize flow needs a session.
- **The `verifiedAt` column default is left in place.** Dropping it is a knex `.alter()`, which rebuilds the table on SQLite — and six tables there carry a foreign key to `accounts` (seven on PostgreSQL, which additionally has one from `actors`), which makes a rebuild a poor trade against a defect the explicit write already closes. `lib/database/sql/account.test.ts` → `leaves verifiedAt null while a confirmation is outstanding` pins the write on both backends, so removing it fails the suite rather than silently restoring the no-op.
