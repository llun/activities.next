...
- **What the rules still do NOT catch:** a helper module that resolves on a driver's behalf, a path built by string concatenation or `path.normalize`, a binding imported from another module, a nested destructure (`const { posix: { resolve } } = path`), and an alias chain longer than `MAX_ALIAS_DEPTH` hops. A reviewer is the enforcement for those. That cap counts **one unit per hop** — a helper delegating within the same hop passes `depth` through rather than adding to it, because charging twice made a three-step rename fall off a budget of five, one step past the rule's own worked example, while the sibling rule reached four. Every predicate also strips `TRANSPARENT_WRAPPERS` — the node types that wrap an expression without changing the value it evaluates to: an optional chain, TypeScript's `as` / `satisfies` / `<T>` / `!` assertions, and a bare generic instantiation (`foo<T>`), all erased at runtime. **Every entry is pinned by a fixture that fails when it is removed, and nothing goes in unpinned** — an unpinned entry is a claim the suite does not check, which is how `TSTypeAssertion` once sat there proving nothing while this bullet promised it. The instantiation entry was also removed once, on the reasoning that `path.resolve` is not generic; the direct `path.resolve<string>` is indeed a TS2635 — **bare**, as an instantiation expression, which is the form this entry is about; add the call parens and `path.resolve<string>(a, b)` is TS2558 instead, same file and same tsconfig, because it is a different node. Two review rounds asked for the code to be "corrected" to TS2558 after writing the call form — but `(path.resolve as unknown as GenericResolver)<string>` compiles clean under `yarn typecheck` and both rules reported nothing for the call while the entry was gone. A type switch stops at any of them, and they NEST and arrive in different POSITIONS, which is why it is a loop over a set rather than a check at one site: oxlint wraps the outermost node of an optional chain, so `path?.resolve` is a `ChainExpression` around the member, `path!.resolve` is a plain member whose OBJECT is the wrapper, and `path.resolve(base) as string` wraps the CALL. An inline `path?.resolve(x)` was always caught either way — the chain wraps the call and the rule reads its callee — which is what hid the whole class. Three separate review rounds each found one member of it (`const r = path?.resolve`, then `path!.resolve(base, x)`, then `full.startsWith(path.resolve(base) as string)`, the plainest-looking of the three), so the set bans the CATEGORY rather than the spellings that happened to turn up. What they replaced — `storagePathCallSites.test.ts`, a raw-text scan — caught strictly less: five review rounds each broke a cleverer version of it, in both directions, because every one of these shapes needs to know what a NAME refers to. Do not reintroduce a text scan for this.
