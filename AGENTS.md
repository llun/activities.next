...
- **Page skeleton alignment (`loading.tsx`).** Route loading skeletons for logged-in pages in `(timeline)` must align with the rendered page so that transition causes no visual jumping of either the header or the content box below it. Look at Timeline (`app/(timeline)/(home)/loading.tsx`), Favorites (`app/(timeline)/favorites/loading.tsx`), and Messages (`app/(timeline)/messages/loading.tsx`):
  - **Reuse `PageHeader` directly.** Never hand-roll a sticky header div or breakout style. Rendering `<PageHeader>` from `@/lib/components/page-header` ensures sticky breakout styling, padding (`px-4 py-4`), bottom border, and total header height (79px) match the loaded page pixel-for-pixel.
