...
- **`getActorStatusesCount` is the one call site that KEEPS the set form, and that is not an oversight to tidy up.** The correlated form costs one recursive-CTE instantiation per Announce row; a LIMIT amortises that over a page and a COUNT cannot amortise it at all. On the same seed it read 40% fewer buffers and still took three times as long — 15,421 buffers / 54ms against 25,752 / 18ms — so unifying it would have undone the `totalItems` work recorded below. `getStatusReplies`, `getStatusReblogs` and `getStatusRepliesCount` did all move to the correlated form: their target sets are one status's replies or boosts, small enough that both forms measure the same (1,451 buffers / 1.4ms against 1,448 / 1.1ms over 240 replies; 3,206 / 1.8ms against 3,040 / 2.0ms over 180 boosts). Keeping two spellings of one subtle predicate is only safe because `publiclyReadableStatusEquivalence.test.ts` runs both over the same rows and compares, and because both build their pointer from the one shared `announceOriginalPointer`.
...
- **`publiclyReadableStatusQueryShape.test.ts` pins the correlated shape, and it is the only thing that would catch a revert.** Putting `getActorStatuses` back on `whereIn(id, buildPubliclyReadableStatusIdsQuery(…))` leaves all 168 tests in `status.test.ts` and the whole equivalence suite green — the two forms select identical rows, which is the point — and fails exactly the two assertions that read the SQL. It also pins the count's set form, so "finishing the unification" has to delete an assertion that explains why not.
