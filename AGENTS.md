...
- **Never put a raw NUL byte in a source file** — for example as a separator inside a string or template literal. Write it as an escape (`\0`) instead, or pick a printable separator.
- It is legal in a JavaScript string and changes nothing at runtime, so **nothing else in the gate notices**: prettier formats it, ESLint accepts it, the build succeeds, and the tests pass. What breaks is every tool that reads the file as text. Git's binary heuristic keys off NUL, so `git diff` prints `Binary files … differ`, GitHub renders **"Binary file not shown"**, `grep`/`ripgrep` skip the file by default, and `git blame` and three-way merge stop working on it.
- This is a review-integrity failure, which is what makes it worth a guard: the reply-by-email change shipped its job file — capability-token resolution, the authorization re-checks, attacker-supplied base64 decoding, the outbound mail path — as an undiffable blob, so the mandated code-review loop could not see a single line of it. Only a byte-level inspection caught it.
- `lib/sourceControlCharacters.test.ts` fails on any tracked non-binary file containing NUL. Genuine binaries are excluded by extension, so a new image or font needs no change; a new binary _format_ does.
