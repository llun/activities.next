...
- **Every filesystem path a local storage driver builds runs through `resolveStorageFilePath` / `assertStorageFilePath` (`lib/services/medias/storagePath.ts`) — never a bare `path.resolve(root, filePath)`.** A stored path is data: it reaches a driver from a `medias` / `fitness_files` row, from a URL path segment, or from an archive entry, and `path.resolve` walks straight out of the root given `../` or an absolute path. The escape is silent, because the read or the unlink simply lands somewhere else on disk. `resolveStorageFilePath` answers null, for a read or delete that has somewhere to return it; `assertStorageFilePath` throws, for a write whose only alternative is creating a file the driver does not own.
- **The rule covers delete and write, not just read.** `LocalFileStorage.getFile` carried the check inline while `deleteFile` beside it resolved and `unlink`ed with none at all. Every caller passed a server-generated `medias.*.path`, so it was latent rather than live — but the asymmetry is the problem: containment was an invariant of the callers, not of the driver, so it held only until someone added a caller, and it made "the storage drivers refuse such a path" false as a claim about the pair (a review of PR #1570 had to narrow a comment in `mediaFileUrl.ts` for exactly that reason). An unlink is the more damaging half. The write paths generate their own names today, so the assert there is the belt rather than the braces, and is applied anyway so confinement is a property of the driver.
- **The module lives under `medias/` and is shared, the same way `medias/fileName` and `medias/quota` already are.** `lib/services/fitness-files/localFile.ts` and `lib/jobs/importStravaArchiveJob.ts` import it too — it replaced the near-identical `fitness-files/path.ts`, so the two drivers cannot drift apart again. Note the root itself resolves to itself; only a path that leaves the root is refused.
...
- Covered by `lib/services/medias/storagePath.test.ts` plus driver-level regression tests in `lib/services/medias/localFile.test.ts` and `lib/services/fitness-files/localFile.test.ts`.
