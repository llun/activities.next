...
- **What the rules still do NOT catch:** a helper module that resolves on a driver's behalf, a path built by string concatenation or `path.normalize`, a binding imported from another module, a nested destructure (`const { posix: { resolve } } = path`), and an alias chain longer than `MAX_ALIAS_DEPTH` hops. A reviewer is the enforcement for those. That cap counts **one unit per hop** — a helper delegating within the same hop passes `depth` through rather than adding to it, because charging twice made a three-step rename fall off a budget of five, one step past the rule's own worked example, while the sibling rule reached four. Every predicate also unwraps a `ChainExpression` first: oxlint wraps the outermost node of an optional chain, so `const r = path?.resolve` reached a type switch as a wrapper and ended the walk, though an inline `path?.resolve(x)` was always caught (there the wrapper is around the CALL and the rule reads its callee). What they replaced — `storagePathCallSites.test.ts`, a raw-text scan — caught strictly less: five review rounds each broke a cleverer version of it, in both directions, because every one of these shapes needs to know what a NAME refers to. Do not reintroduce a text scan for this.
