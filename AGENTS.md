...
- **What cannot be a counter can still be cached for a minute: the outbox root serves `totalItems` through `getCachedActorPublicStatusCount` (`lib/services/statuses/actorPublicStatusCount.ts`) and sends `Cache-Control: public, max-age=60, s-maxage=60`.** The count above is ~100–160ms of CPU over the actor's whole public history, and nothing deduplicated it: on 2026-08-25 a burst of 530 outbox root fetches in 11 minutes each recomputed it and queued behind one another on a 1-vCPU Cloud SQL instance, taking the query's **average wall time to 899ms against ~150ms of real work** — the executions were waiting for CPU, not doing more of it (~4 CPU-sec/sec demand against a capacity of 1; no IO wait, no lock wait). Neither half of the fix is enough alone: without the header every fetch reaches the origin, and without the cache every miss recomputes. A count that lags by up to a minute is within what the collection already promises, since the paragraph above is exactly the argument that it cannot be exact anyway. Three things are deliberate. The cache is keyed `WeakMap<Database, Map<actorId, …>>` — per database so the production singleton caches while each test's throwaway resolves independently (the same shape as `getCachedLocalPublicStatusesCount`), per actor because one instance serves several including the headless signing actor, and **bounded**, because entries expire but nothing sweeps them and remote servers choose which actors get fetched. Errors are never cached; the rejection propagates before anything is written. And the header is on the **root only** — `?page=true` carries `orderedItems` whose per-status visibility is live, so a status hidden mid-window must stop being served at once rather than linger in a shared cache. There is no single-flight dedup: the CDN layer is what absorbs a burst, and a sub-TTL stampede is a different, unmeasured failure mode. `activityPubResponse` grew an `additionalHeaders` pass-through for this; it is **appended** to the negotiated `Content-Type` and `Vary: Accept` rather than overriding them, because `Headers` comma-joins a repeated name instead of replacing it. Note the shared cache only honours `Vary: Accept` if the CDN's cache key includes that header — a content-negotiated route behind a header-blind cache can serve AP JSON to an HTML client.
