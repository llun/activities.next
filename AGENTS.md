...
- **A re-point clears `emailVerified` along with the code, in the same write.** `updateAccountEmail` writes both, because both prove control of the address they were set for and neither may outlive it. Clearing the code alone is not enough for the backfilled cohort: those rows are not pending (the predicate reads `emailVerified`), so a re-point moved their address to an arbitrary one while they stayed verified, and `lib/services/oauth/userinfo.ts` then asserted `email_verified: true` for an address nobody had proven — to any OIDC relying party that links accounts on that claim. The cost is named rather than hidden: that route writes before it sends and answers 500 on a send failure, so a cohort member whose mail fails at that moment is refused until they confirm the new address.
