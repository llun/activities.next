...
- **The deadline bounds one attachment, NOT the export, and that residual is open rather than settled.** Ten minutes times an actor's whole history is still days. The tempting argument — that any aggregate bound would abort a legitimate large download and so trade a stall for silent data loss, which is worse in a backup — is a false dichotomy, and was believed here for a round before review took it apart: a budget scoped to the remote-fetch phase that stops STARTING new downloads once exhausted, never aborting one in flight, degrades through the warn-and-keep-absolute-URL path this code already takes for every refused, oversized or failed attachment. That loses nothing. It is unbuilt, not rejected.
...
- **The residual has a SPACE half as well as a time half, and neither is bounded across the export.** `readResponseArrayBufferWithLimit` buffers, and `Buffer.concat` briefly holds both copies, so one attachment peaks near twice the resolved cap — up to ~2 GiB where an admin has raised `media.maxFileSize` to `MAX_CONFIGURABLE_FILE_SIZE` — while nothing bounds the total written into `os.tmpdir()` across an actor's whole history. Same shape of fix as the time budget, and the same non-destructive degradation. Still strictly better than the unbounded read this replaced.
