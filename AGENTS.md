...
- **Every local username this instance MINTS is lowercased**, by `normalizeUsername` (`lib/utils/normalizeUsername.ts`, the sibling of `normalizeEmail` and deliberately as minimal: trim then lowercase). It runs in three places, layered the same way email normalization is: `localUsernameSchema` (both creation routes), `registerAccount` (so a direct service call cannot bypass the schema), and `createAccount`/`createActorForAccount` (the one place a local actor row is written, which is what makes it unbypassable). The DB layer normalizes into a local variable that feeds BOTH the `username` column and `getLocalActorId`, so the column and the id are derived from one value and cannot drift.
- **Folding in `localUsernameSchema` runs BEFORE the reserved-name refine, and that ordering is load-bearing.** `isFederationSigningActorUsername` is a case-sensitive `startsWith('__instance__')`, so `__INSTANCE__` passed the check and minted a confusable neighbour of the instance actor at a _different_ id. It is also before `.max()`, so the length check reads the value actually stored — a fold can change a string's length. `LOCAL_USERNAME_PATTERN` still names `A-Z` on purpose: it describes what a caller may SEND, and by the time it runs there is none left.
- **Every username LOOKUP folds**, through `findActorRowByUsername` (`lib/database/sql/utils/usernameMatch.ts`), which backs both `getActorFromUsername` and `isUsernameExists`. That covers `/@user`, WebFinger, mentions (which go through WebFinger), `/api/v1/accounts/lookup`, both search routes, `authorize_interaction` and the actor archive. `isUsernameExists` folding is what stops a second `Alice` from being minted beside an existing `alice`.
- **It is exact-match-first, then folded — NOT a single `lower(username) = ?`, for two independent reasons.** (1) SQL `lower()` and JS `toLowerCase()` are not the same function: SQLite's builtin folds ASCII only, so a remote actor named `Фёдор` has its stored value fold to `Фёдор` while the JS side folds the identical input to `фёдор`, and a fold-only query stops finding a row that resolves today. Trying exact first means no lookup that works now can break, on any backend, for any charset. (2) Local actors minted before normalization keep their casing — their ids are already federated, so they are deliberately NOT migrated — which means an instance can hold both `Alice` and `alice`, and `/@Alice` must not start resolving to `alice`. "Exact wins" says that without a tiebreak clause. The folded arm orders by `createdAt` then `id` so an unmatched casing resolves to whoever claimed the name first, rather than to whatever the index yields.
...
- **`domain` matching inside the lookup stays case-sensitive.** Callers lowercase it themselves (`parseAccountHandle` already does), and `getWebFingerResponse` carries its own exact-then-lowercased domain fallback; folding it in the helper would silently change which host a handle resolves on, which is a different question from this one.
