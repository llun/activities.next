...
- **What the rules still do NOT catch:** a helper module that resolves on a driver's behalf, a path built by string concatenation or `path.normalize`, a binding imported from another module, and a nested destructure (`const { posix: { resolve } } = path`). A reviewer is the enforcement for those. What they replaced — `storagePathCallSites.test.ts`, a raw-text scan — caught strictly less: five review rounds each broke a cleverer version of it, in both directions, because every one of these shapes needs to know what a NAME refers to. Do not reintroduce a text scan for this.
