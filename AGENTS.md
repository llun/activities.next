...
- **Existing accounts are grandfathered, but by the migration, NOT by leaving `verifiedAt` alone.** Nothing backfills `verifiedAt` — so on its own the `verificationCode` half of the check still reaches every legacy pending account, and for the pre-2026-03-20 cohort described above that meant losing a working password login with no way back (the resend endpoint needs a credential, which is what is being refused). `20260828140000_clear_stale_verification_codes` is what actually grandfathers them. For an account registered after that date, the only tokens the gate withdraws are ones minted through the hole being closed — `registerViaApi` is the sole path that mints a user token for an unconfirmed account, since credential sign-in is refused and the authorize flow needs a session.
- **The `verifiedAt` column default is left in place.** Dropping it is a knex `.alter()`, which rebuilds the table on SQLite — and **seven** tables carry a foreign key to `accounts` on both backends (`actors`, `oauthClient`, `oauthRefreshToken`, `oauthAccessToken`, `oauthConsent`, `passkey`, `twoFactor`), `actors` among them, which makes a rebuild a poor trade against a defect the explicit write already closes. Count these by matching case-insensitively and on both quoting styles: `actors` is emitted as `CREATE TABLE IF NOT EXISTS "actors"` with uppercase `REFERENCES`, so a grep written for backticks and lowercase silently reports six. `lib/database/sql/account.test.ts` → `leaves verifiedAt null while a confirmation is outstanding` pins the write on SQLite and PostgreSQL, so removing it fails the suite rather than silently restoring the no-op. It cannot speak for MySQL-compatible clients: knex emits the column as a bare `TIMESTAMP DEFAULT CURRENT_TIMESTAMP`, and under the legacy `explicit_defaults_for_timestamp = OFF` (MariaDB's default) such a column is implicitly `NOT NULL` and an inserted `NULL` is CONVERTED to the current timestamp. Unverified — there is no MySQL in the harness — and it would cost only the defence-in-depth column, since `verificationCode` is what the gate actually reads.
