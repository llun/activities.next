...
- **`storagePathCallSites.test.ts` is what makes the rule enforceable rather than documentary** — the same idea as `statusVisibilityCallSites.test.ts` and `clientModuleBoundary.test.ts`. It bans `path.resolve` and `path.join` **outright** in the two drivers, and flags a `startsWith` containment check against a resolved root anywhere in `app/`, `lib/` or `scripts/` — inline or with the root pulled into a variable first, scanning each file as one string. The outright ban is deliberate and the second attempt: a first version tracked `this._config.path` and its aliases, and three ordinary refactors walked through it — a destructured rename, a nested call in the argument list, and **prettier wrapping the call across lines**, which meant step 1 of the commit gate could disarm the guard. Neither driver needs either constructor (only `extname`/`dirname`, which read a path rather than build one), so a driver that suddenly needs one should fail this test and have the exemption discussed. Every other test here proves the helper and today's call sites; only this one can catch a call site added tomorrow.
...
- **A path recovered from a URL is a DIFFERENT boundary — do not consolidate the two.** `scripts/backup/actorArchive.ts`'s `copyProfileImage` joins a path recovered from the actor's own `iconUrl` (an unvalidated `z.string()` on `POST /api/v1/accounts/profile`) into the staging directory, which is a live traversal being fixed on the open PR #1569 — with `isTraversingStoragePath` inside `getMediaPathFromFileUrl`, **not** with this module. That is correct: `isTraversingStoragePath` asks whether a recovered string is SHAPED like traversal, needs no root, runs after decoding and deliberately over-refuses `\` and NUL for Windows operators; `resolveStorageFilePath` asks whether a path resolves inside ONE named root and is meaningless without it. They are complementary.
