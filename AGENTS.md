...
- **The SQL test is `privateKey IS NOT NULL AND privateKey <> ''`, and it belongs to `whereLocalActor` (`lib/database/sql/utils/localActor.ts`) — never write either half by hand.** Actor rows written by earlier remote-recording paths store the key they do not have as an **empty string**, not NULL. A null-only check therefore counts them as local: on production that was **216 of the 221** rows it matched, all on remote domains, which made the logged-out local timeline majority-remote and had `getLocalFollowersForActorId` treating every domain those actors live on as a local domain. `20260821120000_normalize_empty_actor_private_key.js` rewrites the stored `''` to NULL; the `<> ''` half stays anyway, for databases restored from an older dump. Applied via knex's `modify`, which works mid-chain and inside a `whereExists` callback: `query.modify(whereLocalActor, 'actors.privateKey')`.
...
- **The local public timeline must NOT join `actors`; it passes the local actor ids in as literal values** (`localPublicStatusesQuery`, `lib/database/sql/timeline.ts`). Joining on `actors.id` — a unique key — leaves the planner no statistic correlating an actor with how much it posts, so it assumes an even spread and under-estimates the eligible rows by ~48x. Below a page of 24 it still picked the ordered index scan; at the API's own default of 30 it flipped to sequentially scanning `actors`, gathering every status those actors wrote and sorting the lot. Measured on a local PostgreSQL 17 seeded to production's shape, returning 30 rows: **literal ids 176 buffers, semi-join on `actors` 16,202, inner join 16,209.** Both join forms lose early termination at _every_ page size, and widening the partial `actors_local_idx` predicate to match does not recover it. **Adding `<> ''` to the join form without switching to literal ids makes this query dramatically worse** — the two halves of that change are not separable. `localPublicQueryShape.test.ts` pins the shape, because every result-based assertion passes against the slow one.
