...
- Common triggers and the docs they touch:
  ...
  - Changes to AGENTS.md rules themselves → keep `CLAUDE.md` in sync, and the PR checklist in `.github/PULL_REQUEST_TEMPLATE.md`. **`AGENTS.md` and `CLAUDE.md` are the only agent instruction files in this repository.** Per-tool variants were tried and removed rather than maintained in parallel — `.cursor/rules/agents.mdc` in #1424, `GEMINI.md` and `.github/copilot-instructions.md` after that. Each one was a partial restatement that went stale on its own schedule and that nothing read often enough for anyone to notice. Do not reintroduce one: a tool that looks for its own filename will find `AGENTS.md` if pointed at it, and one more copy of the rules is a liability, not coverage. `CLAUDE.md` earns its place by being the condensed reminder list loaded at task start; it cites AGENTS.md sections as ``See **Section Name** in `AGENTS.md`.`` so a reader can always reach the full rule
