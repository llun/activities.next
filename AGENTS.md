...
- **The `verifiedAt` column default is left in place.** Dropping it is a knex `.alter()`, which rebuilds the table on SQLite — and six tables carry a foreign key to `accounts`, which makes a rebuild a poor trade against a defect the explicit write already closes. `lib/database/sql/account.test.ts` → `leaves verifiedAt null while a confirmation is outstanding` pins the write on both backends, so removing it fails the suite rather than silently restoring the no-op.
