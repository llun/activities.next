...
- **An attachment the Mastodon serializer cannot describe is `type: "unknown"`, never `null`.** `getMastodonAttachment` covers image and video; an `audio/mp4` upload (an accepted type) has no stored duration for Mastodon's `Audio` shape and a remote GIF no `gifv` metadata. Emitting `null` broke the entity — `media_attachments` is an array of MediaAttachment — and, worse, left that attachment with no id for a client to name, so an edit echoing back the ids it could see read the unnameable one as a removal and `updateNote` deleted it. `unknown` carries the id, url, description and blurhash without claiming dimensions the row does not have.
