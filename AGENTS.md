...
- **A term this instance WRITES onto a Note must be declared in the context it sends, not only aliased inbound.** `CANONICAL_CONTEXT` governs what we accept; `NOTE_ACTIVITY_CONTEXT` (`lib/activities/noteContext.ts`) is what we emit, and every Note-carrying surface uses it — `sendNote`'s Create, `sendUpdateNote`'s Update, the outbox page, and the AP representation of a single status. Sending the bare ActivityStreams URL instead is not a smaller context, it is a lossy one: that context sets `"@vocab": "_:"`, so every undefined key expands to a blank node the receiver drops. This was invisible for a long time because Mastodon reads the JSON without processing it, while a conformant receiver — GoToSocial, or another activities.next, whose inbox runs `compactActivityPub` — silently lost `interactionPolicy` (emitted on EVERY Note, so a plain text post federated with no quote policy), the quote aliases, and an attachment's `blurhash`/`focalPoint`. `focalPoint` needs `"@container": "@list"` or a processor is free to reorder the `[x, y]` pair. `lib/activities/jsonld/index.test.ts` round-trips a Note through the emitted context and the inbound processor; it is the only thing that catches a revert, since nothing about the outgoing JSON looks wrong.
