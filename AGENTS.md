...
- **A client-supplied id compared against a numeric column must be coerced first — PostgreSQL turns a bad id into an error, not a miss.** `medias.id` (and `attachments.mediaId`) are `integer` on PostgreSQL, so `where('medias.id', 'abc')` raises `invalid input syntax for type integer` and a 404 becomes a 500. SQLite's dynamic typing just matches nothing, so the default test run never sees it: `TEST_DATABASE_TYPE=sqlite` is what CI pins, and only `TEST_DATABASE_TYPE=pg` catches this class of bug. In `lib/database/sql/media.ts` every method that **compares** a `mediaId` against `medias.id` runs it through `toMediaRowId` first, so the caller reports "not found" without touching the database. (`createAttachment` **writes** `mediaId` rather than comparing it, and is not guarded — every production caller hands it an id read back out of `medias`.) The guard is shape-checked and range-bounded on purpose, not a bare `Number()`: `Number()` also accepts `'0x10'` (16), `'0b101'` (5), `'1e3'` (1000), `'+12'` and `' 12 '`, each of which resolves a _different_ row than the id names, while an id above `integer`'s 2147483647 raises `value out of range for type integer` — the same 500 in a new costume. A trailing all-zero fraction (`'12.0'`) is the one accepted alias: it names row 12 unambiguously and is the form existing **SQLite** deployments already hold in `attachments.mediaId`, because that column is `varchar` there and a media id bound as a JS number reaches it through SQLite's REAL→TEXT conversion. Rejecting it would 422 an edit of any affected status. Apply the same treatment to any new query that compares a caller-supplied value against a numeric column, and give test fixtures values the column can actually hold.
