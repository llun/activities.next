...
- **Never render `error_description`.** It is free text better-auth puts in the query string, and anyone can hand-craft a link to `/auth/error` with any value in it — prose we did not write, shown on our own auth card, is a ready-made phishing surface. The page renders copy keyed off an allow-listed, token-shaped `error` code (`resolveAuthErrorContent` / `sanitizeAuthErrorCode` in `lib/services/auth/errorPage.ts`) and falls back to generic copy for anything unmapped. A bounded slice of the description is logged, not displayed. Codes are looked up with `Object.hasOwn` — a bare lookup answers `constructor`/`toString` with an inherited function, which is truthy and would render an empty card.
...
- **`access_denied` does not come here.** A user declining consent is reported to the client's `redirect_uri` per RFC 6749 §4.1.2.1; the code is mapped on the page only for a client that forwards it back by hand.
