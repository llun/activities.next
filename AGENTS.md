...
- **Recover a stored media path from a URL with `getMediaPathFromFileUrl` (`lib/services/medias/mediaFileUrl.ts`) — never by checking the `/api/v1/files/` prefix on its own.** That route is this project's own, so **every other activities.next instance serves its attachment URLs under exactly that path**. A path-only check therefore reads a remote instance's URL as one of our storage paths: the lookup misses in storage, and at a caller that treats "not local" as "fetch it over HTTP instead" the download branch is skipped entirely, so the archive or backfill references a file it never obtained. `scripts/backup/actorArchive.ts` (attachment URLs plus the actor's `iconUrl`/`headerImageUrl`) is migrated to this shared, host-aware parser. `scripts/maintenance/backfillMediaBlurhash.ts` hit the same bug independently and **still carries it on `main`** (`row.url.indexOf('/api/v1/files/')` there, whose `else if` structure means the HTTP-fetch fallback is never reached for a mis-detected URL) — it is fixed on the open PR #1559 with its own local copy of the same logic, and whichever of the two lands second should drop that copy in favor of this shared helper. It is the inverse of `getMediaFileUrl` and lives beside it.
...
- **`storagePathCallSites.test.ts` is what makes the rule enforceable rather than documentary** — the same idea as `statusVisibilityCallSites.test.ts` and `clientModuleBoundary.test.ts`. In the two drivers it pins the path import to exactly one `import path from 'path'` — counting every reference to the module, because a REGEX over the import line matched nothing at all when the import was wrapped and passed vacuously while the file called a bare `resolve(…)` — and with the import pinned, banning `path.resolve`, `path.join` and computed `path[…]` is exhaustive for those two files. Neither driver needs any of it (only `extname`/`dirname`, which read a path rather than build one), so a driver that suddenly does should fail this test and have the exemption discussed. It scans with comments stripped, so documenting the rule inside a driver does not trip it. Repo-wide it flags the inline `startsWith(path.resolve(…))` containment idiom. **It is deliberately scoped to what a text scan can decide correctly**, and every earlier version that reached for more sprang a new leak — disarmed by prettier reformatting a call, then by `path['resolve']`, then failing on an unrelated variable that merely shared a name, then passing vacuously on a wrapped import. Its own header comment carries that history; what belongs here is the rule. The named residual: pulling the resolved root into a variable before the `startsWith` is NOT caught, because that needs to know which declaration a name refers to — an AST rule in `lint/agentsRules.mjs` (`no-component-fetch` is the precedent) is the right home for that, and is a follow-up. Every other test here proves the helper and today's call sites; only this one can catch a call site added tomorrow.
...
- **"Is this string shaped like traversal?" is a DIFFERENT question from "does this resolve inside THIS root?" — do not fold one into the other.** A shape check takes no root, has to run after decoding, and is where over-refusing `\` and NUL for Windows operators belongs; `resolveStorageFilePath` takes exactly one root and is meaningless without it. A path recovered from a URL needs the first (`scripts/backup/actorArchive.ts`'s `copyProfileImage` joins one straight into the staging directory, from an actor's own unvalidated `iconUrl`, and is the live traversal PR #1569 is open against); a path handed to a storage driver needs the second.
