...
- The context is UX only. Every limit is enforced server-side too (`validateStatusContentLimits` on the status create/edit routes, `exceedsMaxMediaUploadSize` on every upload path); a client that is out of date can only be optimistic, never permissive. `posts.maxMediaAttachments` is the exception: the create and edit status routes bound the attachment count by the fixed `MAX_STORED_MEDIA_ATTACHMENTS` ceiling, not by this resolved value, so a stale or missing provider can let through more media than the admin configured (up to that fixed ceiling). See **Database-backed server settings** in `docs/environment-variables.md` for the full explanation. When a new surface can author or upload, put it under the provider and read the limit from it.
