...
- **A re-point clears every proof about the old address, in the same write.** `repointUnconfirmedAccountEmail` writes the rotated code, `emailVerified: false`, `verifiedAt: null` and `emailVerifiedAt: null` together, because each proves control of the address it was set for and none may outlive it. Clearing the code alone is not enough for the backfilled cohort: those rows are not pending (the predicate reads `emailVerified`), so a re-point moved their address to an arbitrary one while they stayed verified, and the id_token's `email_verified` claim (`lib/services/auth/auth.ts`, which reads `emailVerified`) then asserted a verified address to any OIDC relying party that links accounts on it. It was NOT `/oauth/userinfo` that leaked this — that route computed the claim from `verifiedAt` until it was moved onto `emailVerified`, so clearing the flag could not have changed what it answered. Two costs are named rather than hidden. That route writes before it sends and answers 500 on a send failure, so a cohort member whose mail fails at that moment is refused until they confirm the new address. And `verifyAccount` restores `verifiedAt` and `emailVerified` but not `emailVerifiedAt` — only `verifyEmailChange` writes that one — so an account that had previously changed its address loses the `/account` "Verified" badge for good once it re-points.
