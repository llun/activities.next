...
- **`isSameActivityPubOrigin` (`lib/utils/activitypub.ts`) fails CLOSED and is the shared spelling of a check five modules still carry inline** as `sameHost`/`sameAuthority` (`processForwardedActivityJob`, `verifyRemoteQuote`, `persistInboundQuoteEdge`, `handleQuoteRequest`, `handleQuoteResponse`); those copies should collapse onto it in the dedup pass. Failing closed matters more than it looks: a blank node, an empty string and an unparseable id all match **nothing, including themselves**, so no pair of degenerate ids can ever compare equal the way two `normalizeActivityPubUri(null)` results would.
