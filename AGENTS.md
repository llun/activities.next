...
- **`storagePathCallSites.test.ts` is what makes the rule enforceable rather than documentary** — the same idea as `statusVisibilityCallSites.test.ts` and `clientModuleBoundary.test.ts`. It fails any `path.resolve`/`path.join` in the two drivers taking the configured root (or a local alias of it, since both write paths bind `const uploadPath = this._config.path` first), and any `startsWith(path.resolve(…))` anywhere in `app/`, `lib/` or `scripts/` — the separator-less containment idiom, which reads as correct at a glance. Every other test here proves the helper and today's call sites; only this one can catch a call site added tomorrow.
...
- **`scripts/backup/actorArchive.ts` has a live instance of this class that is NOT fixed here.** `copyProfileImage` joins a path recovered from the actor's own `iconUrl` — an unvalidated `z.string()` on `POST /api/v1/accounts/profile` — into the staging directory with no containment check, so an actor can have an arbitrary server file copied into their export. It is being fixed on the open PR #1569; whichever lands second should use `assertStorageFilePath` rather than adding a fourth local copy.
