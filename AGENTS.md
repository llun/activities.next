...
- **Count straight off `buildPubliclyReadableStatusIdsQuery`; do not filter `statuses` by it.** The subquery is already scoped to the caller's target set, so `where actorId = ? and id in (<subquery>)` re-applies a predicate the subquery has already decided — at one `statuses_pkey` lookup per returned id, plus a sort for the `COUNT(DISTINCT)`. On a PostgreSQL 18 seed shaped like production (500k statuses, an actor with 2,000 of them, half Announces) that outer re-join alone was 7,884 of 37,731 shared buffers.
- **The announce chain is followed through one pointer column, `COALESCE("originalStatusId", content)`, not an `OR` of the two forms.** Modern Announce rows set both; legacy rows only ever wrote the original's id into `content`. Spelling that as `originalStatusId = id OR (originalStatusId IS NULL AND content = id)` makes PostgreSQL build a `BitmapOr` of two `statuses_pkey` probes per row, and forces the recursion's working table to carry the wide `content` text for every row — including the Notes that never follow a pointer. The `CASE` guard on `type` is what keeps a Note's body out of that column. Together with the point above: **37,731 → 27,018 buffers and 71 → 36 ms** for the count, 37,734 → 34,905 buffers for the outbox page.
- **knex mis-orders positional bindings when a term of a `UNION`'d CTE mixes a bound raw in its select list with a bound subquery in its `FROM`** — the `FROM`'s value is emitted first, so the two swap silently. That is exactly the shape of `buildPubliclyReadableStatusIdsQuery`, so the `Announce` literal in the chain pointer is escaped into the fragment (`database.raw('?', [...]).toString()`) instead of bound; identifier placeholders (`??`) are resolved at compile time and never reach the binding array. Both queries still compile and run, they just compare the wrong things, so this is guarded by compiled-SQL assertions in `lib/database/sql/status.test.ts` rather than left to behaviour tests.
