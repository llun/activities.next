...
- The **design system is the source of truth** for page chrome. Settings-style sections (settings, fitness) lead with a **shared section header in the full-width sticky chrome** (`<PageHeader title="Settings" … contentWidth="wide" />` rendered **outside** `PageHeaderSectionProvider`), then a **dropdown sub-navigation on every breakpoint — desktop included** (outline trigger with the active tab's `text-primary` Lucide icon + sentence-case label + chevron, `w-full` on mobile and `sm:w-64` from `sm` up), with each page's own title rendered in **section mode** below. **Do NOT reintroduce a vertical icon nav rail** on desktop for these sub-pages — the rail was removed in favor of the dropdown so the content always spans the full width. No horizontal tabs inside the page header either. Admin-style sections keep the sticky-header sub-nav via `PageSubnavProvider`. See the **Page Header & Sub-Navigation** section in `AGENTS.md`.
