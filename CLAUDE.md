...
- The **design system is the source of truth** for page chrome. Settings-style sections (settings, fitness settings) lead with a **shared section header in the full-width sticky chrome** (`<PageHeader title="Settings" … contentWidth="wide" />` rendered **outside** `PageHeaderSectionProvider`), then a **vertical icon nav rail** beside the content (orange-wash active, Lucide icons, sentence-case labels, dropdown below `lg`), with each page's own title rendered in **section mode** below — not horizontal tabs inside the page header. Admin-style sections keep the sticky-header sub-nav via `PageSubnavProvider`. See the **Page Header & Sub-Navigation** section in `AGENTS.md`.
