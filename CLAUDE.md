...
- The **design system is the source of truth** for page chrome. Settings-style sections (settings, fitness settings) use a **vertical icon nav rail** beside the content via `PageHeaderSectionProvider` (orange-wash active, Lucide icons, sentence-case labels, dropdown below `lg`), not horizontal tabs inside the page header. Admin-style sections keep the sticky-header sub-nav via `PageSubnavProvider`. See the **Page Header & Sub-Navigation** section in `AGENTS.md`.
