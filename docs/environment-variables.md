...
| `ACTIVITIES_FITNESS_MAP_PROVIDER`                            | Fitness map provider: `apple`, `mapbox`, or `osm`. When unset or set to an unknown value the provider is inferred for back-compat: `mapbox` if `ACTIVITIES_FITNESS_MAPBOX_ACCESS_TOKEN` is set, otherwise `osm`. If `apple` is selected but its credentials are incomplete, it falls back to `osm`. Changing the provider requires an app restart because the resolved CSP is cached for the process lifetime. |
