...
| `ACTIVITIES_ALLOW_REMOTE_MEDIA_DOMAINS` | Optional JSON array of remote media origins allowed by runtime `img-src` and `media-src` CSP for browser-loaded federated status images, avatars, emoji, video, and audio. Next image optimization is disabled and remote image patterns are build-time static, so this setting is enforced at request time instead of during `next build`. When unset, federated remote media defaults to broad HTTPS browser loading; when set, broad `https:` is replaced by the configured origins. |
