...
| `ACTIVITIES_MEDIA_STORAGE_HOSTNAME`          | Public media hostname/CDN used to serve stored media files.                                                                                                                              |
| `ACTIVITIES_MEDIA_STORAGE_ENDPOINT`          | S3-compatible API endpoint used for storage operations and browser presigned uploads (for services like MinIO, DigitalOcean Spaces, Cloudflare R2).                                      |
...
| `ACTIVITIES_FITNESS_STORAGE_HOSTNAME`                       | Public fitness file hostname/CDN used to serve stored fitness files.                                                                                                                                                                                               |
| `ACTIVITIES_FITNESS_STORAGE_ENDPOINT`                       | S3-compatible API endpoint used for storage operations and browser presigned uploads.                                                                                                                                                                              |
...
| Variable                         | Description                                                                                                                                                                                                                                                                                                                |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ACTIVITIES_ALLOW_MEDIA_DOMAINS` | Optional JSON array of remote image origins allowed by runtime `img-src` CSP. Next image optimization is disabled and remote image patterns are build-time static, so this setting is enforced at request time instead of during `next build`. When unset, federated remote images default to broad HTTPS browser loading. |
