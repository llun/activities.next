...
| `ACTIVITIES_TRUSTED_HOSTS`       | No       | JSON array of additional public hosts accepted from `X-Forwarded-Host` and `X-Activity-Next-Host`. Also lets WebFinger, account search, account lookup, and the server-rendered profile and status pages resolve `user@<trusted-host>` to the canonical local actor, so clients reaching the instance via an alias host can load the local account. |
