...
| `ACTIVITIES_FITNESS_MAPBOX_ACCESS_TOKEN`                     | Mapbox API token for map rendering. Required when `ACTIVITIES_FITNESS_MAP_PROVIDER=mapbox` (an empty token falls back to `osm`). Only public `pk.*` tokens are passed to browser maps; secret `sk.*` tokens stay server-side and the browser falls back to `osm`.                                                                                                                                                                                |
...
| `ACTIVITIES_FITNESS_APPLE_MAPS_TEAM_ID`                      | Apple Developer team ID used to sign Apple Maps (MapKit JS) tokens. Required when `ACTIVITIES_FITNESS_MAP_PROVIDER=apple`.                                                                                                                                                                                                                                                                                                                       |
| `ACTIVITIES_FITNESS_APPLE_MAPS_KEY_ID`                       | Apple MapKit JS key ID (Key ID of the MapKit private key). Required when `ACTIVITIES_FITNESS_MAP_PROVIDER=apple`.                                                                                                                                                                                                                                                                                                                                |
| `ACTIVITIES_FITNESS_APPLE_MAPS_PRIVATE_KEY`                  | Apple MapKit JS private key (PEM). Required when `ACTIVITIES_FITNESS_MAP_PROVIDER=apple`. A single-line `\n`-escaped PEM is accepted and expanded into a real multi-line key.                                                                                                                                                                                                                                                                    |
...
Fallback rule: if the selected provider's credentials are missing or invalid, the
resolver silently falls back to `osm` rather than failing to render. The same applies
when the variable is unset or holds an unknown value, except that a configured Mapbox
token then selects `mapbox` for backward compatibility. Because the Content-Security-Policy
...
**Regenerate maps** (Settings → Fitness, `POST /api/v1/fitness/general/regenerate-maps`)
after switching providers to re-render them.
