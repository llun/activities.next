...
An empty `ACTIVITIES_MEDIA_STORAGE_PATH` or `ACTIVITIES_FITNESS_STORAGE_PATH`,
or an empty bucket/region on either family, disables that storage with a warning
instead of starting a broken backend. An empty path would otherwise resolve
relative to the process working directory, putting uploads in the application
root — and for media, serving that directory over `/api/v1/files`. Fitness
storage falls back to the `ACTIVITIES_MEDIA_STORAGE_*` variables when its own
are unset, and the guard applies on both paths.
