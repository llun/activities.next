...
Leaving one of them **unset** while its storage type is configured is different, and still a hard startup failure — unless another required variable for the same storage type is blank, which disables storage before validation runs. Note that this applies to blank values only: the storage **type** variables themselves are not trimmed, so `ACTIVITIES_MEDIA_STORAGE_TYPE=' fs '` is an unrecognised type and disables storage.
...
Setting `ACTIVITIES_FITNESS_STORAGE_TYPE` at all opts out of the media-storage fallback. If the type is set but unusable — a blank required value, or a type this app does not recognise — fitness storage is disabled and a warning names the variable; it does **not** quietly write fitness files into the media bucket.
