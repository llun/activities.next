...
- **Page skeleton alignment (`loading.tsx`).** Route loading skeletons for logged-in pages in `(timeline)` must align with the rendered page so that transition causes no visual jumping of either the header or the content box below it:
  ...
  - **Home timeline boundaries.** On `(home)` below `md` (768px), the sticky `PageHeader`'s existing bottom border meets the composer with no white band, and the composer meets the feed through one full-viewport-width, 1px `border`-token divider. Keep the outer timeline stack's spacing for announcement-to-header and feed-to-pagination boundaries, and mirror the same geometry in `(home)/loading.tsx` so loading, empty, refresh, and populated states do not jump. At `md` and above, keep the separate rounded, shadowed composer and feed cards with their 24px gaps in the centered `max-w-content` column. Do not wrap the header in a narrower containing block or change the shared `MOBILE_FEED_SURFACE_CLASS`; this is home-boundary spacing only.
  - **Mobile timeline pagination spacing (`LoadMoreButton`) and Scroll to top button (`ScrollToTopButton`).** Following the removal of the mobile bottom navigation bar, timeline feeds end at the bottom viewport edge. The shared `LoadMoreButton` container applies `max-md:pt-6 max-md:pb-[calc(env(safe-area-inset-bottom,0px)+1.5rem)]` (with `py-4` on desktop) so that the load more button and any load errors are moved down comfortably from the final post while clearing the Safari mobile address bar and home indicator. It must remain in the normal document flow (never `fixed`) so it functions properly as an IntersectionObserver sentinel. In parallel, `ScrollToTopButton` uses fixed positioning with `bottom-[calc(env(safe-area-inset-bottom,0px)+0.75rem)]` to preserve safe-area clearance above the home indicator while maintaining approximately 12–16 CSS pixels of visible clearance above Safari's collapsed bottom address bar without floating excessively high over post content.
...
- **Timeline grouping and nested status threads.**
  - `TimelineFeed` (`lib/components/posts/timeline-feed.tsx`) renders grouped home timelines, connecting related posts into visual threads and multi-author conversations with connector rails, accessible badges, compact collapsed middle runs for long chains, and an optional boost carousel (`BoostCarousel`).
