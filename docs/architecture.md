...
- Destructive **text** and link labels in dark mode use `text-destructive-text`
  (or `text-destructive`, which maps to it in `@layer utilities`), not raw
  dark `--destructive`. Dark `--destructive` is `hsl(0 62.8% 30.6%)` (#7F1D1D)
  to preserve 9:1 white-on-destructive contrast for button and badge backgrounds
