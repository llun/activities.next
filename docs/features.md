...
- ✅ **Automated alt text** — Generate accessibility descriptions for uploaded images and videos from a configured OpenAI-compatible vision endpoint (`ACTIVITIES_ALT_TEXT_*`) when the client sends none. Video descriptions come from the representative preview frame, so a clip opening on a black or blank frame still gets a meaningful description; extraction or vision failures degrade to an upload with no generated description rather than failing the upload
