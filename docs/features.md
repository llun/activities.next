...
- ✅ **Lists** — Create and manage timeline lists, their members (accounts you follow, plus yourself — `POST /api/v1/lists/:id/accounts` accepts your own account without a follow, and a `following=true` account search — `GET /api/v1/accounts/search` or `GET /api/v2/search` — includes you, as Mastodon's do), replies policy, and exclusive flag
