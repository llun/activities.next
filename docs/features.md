...
- ✅ **Pluggable fitness map provider** — Choose `apple` (MapKit JS + Apple Maps Snapshots), `mapbox` (Mapbox GL JS + Static Images API), or `osm` (keyless MapLibre / OpenFreeMap) with `ACTIVITIES_FITNESS_MAP_PROVIDER`; missing or invalid credentials fall back to `osm`. Stored route-map images keep the previous provider's style until the user runs **Regenerate maps** (`POST /api/v1/fitness/general/regenerate-maps`). With the `apple` provider, per-activity route-map images are rendered by Apple Web Snapshots; a heatmap embed image with more segments than Apple's overlay ceiling allows is offered the coarser stored geometry instead, and falls back to the built-in SVG heatmap renderer only when that does not fit either
