...
- ✅ **Automated alt text** — Generate accessibility descriptions for uploaded images and videos from a configured OpenAI-compatible vision endpoint (`ACTIVITIES_ALT_TEXT_*`) when the client sends none. Video descriptions come from the representative preview frame, which is more likely than frame 0 to show the scene instead of a black or blank opening; a vision or preview-extraction failure degrades to an upload with no generated description rather than failing the upload. Storing the video's own poster frame is still required and fatal if the clip cannot be decoded
...
- ✅ **Lists** — Create and manage timeline lists, their members, replies policy, and exclusive flag. Members are accounts you follow plus yourself: `POST /api/v1/lists/:id/accounts` accepts your own account without a follow, and a `following=true` account search — `GET /api/v1/accounts/search` or `GET /api/v2/search` — includes you, as Mastodon's do. Every other member needs an accepted follow, whereas Mastodon ≥ 4.2 also accepts an account you have only requested to follow
