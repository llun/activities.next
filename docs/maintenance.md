...
- **A video's representative preview frame comes from ffmpeg's `thumbnail` filter, not frame 0.** `extractVideoImage` (`lib/services/medias/extractVideoImage.ts`) runs `ffmpeg -vf thumbnail -frames:v 1 -update 1`, so ffmpeg analyses batches of candidate frames and emits the most visually representative one. Taking the first decodable frame instead — the previous behaviour — produced a black or blank poster, and that same frame is what the alt-text model was asked to describe. `-update 1` is required because a single image2 output has no `%0Nd` sequence pattern in its name. Every extraction goes through `extractVideoPreviewFrame` (`lib/services/medias/videoPreview.ts`), which writes a server-named temp copy first; see **Uploaded file names are untrusted input** above for why the name takes nothing from the upload.
