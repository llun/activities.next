...
>
> **Note:** `recreateFitnessRouteHeatmaps.ts` soft-deletes the actor's heatmap rows and queues a generation job per variant. The all-activities/all-time run also rebuilds that actor's **tile pyramid**, which is what the interactive maps and the share image draw street-level detail from, so a rebuild is heavier than the queued-job count suggests. While it runs the pyramid is `generating`, not `completed`, so nothing serves tiles — and the script has already soft-deleted the heatmap rows, so there is no stored geometry to fall back to either. A share token resolves only to a live, completed row, so shared links and embeds answer 404 for the whole window rather than publishing a partial map. Expect them to be unavailable until the rebuild finishes.
>
