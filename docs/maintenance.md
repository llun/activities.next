...
>
> **Note:** `recreateFitnessRouteHeatmaps.ts` soft-deletes the actor's heatmap rows and queues a generation job per variant. The all-activities/all-time run also rebuilds that actor's **tile pyramid**, which is what the interactive maps and the share image draw street-level detail from, so a rebuild is heavier than the queued-job count suggests. While it runs the pyramid is `generating`, not `completed`, so nothing serves tiles: every surface falls back to the stored geometry until the build stamps itself completed. A share whose heatmap row is still being regenerated answers 404 for that window rather than publishing a partial map, so expect shared links and embeds to be unavailable until the rebuild finishes.
>
