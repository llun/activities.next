...
- **Service reminders** compare the derived totals against `alertDistanceMeters` (shoes) and `serviceDistanceMeters` (components), recording the distance they fired at in `lastAlertedDistanceMeters` so each crossing notifies exactly once. Raising a threshold, replacing a part, or retiring and unretiring re-arms it. They are evaluated when a total can change — when an activity finishes processing, and when one is attributed by hand — because this instance has no recurring job infrastructure; the queue can delay a message but not repeat one.
