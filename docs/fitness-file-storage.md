...
Nothing builds the two pyramid tables yet — `fitness_route_heatmaps` still serves every heatmap, and the tiler, the generation job, the serving API and the client's zoom-aware fetching land in later changes. They are already wired into deletion, though: clearing an actor's heatmaps drops the pyramid row **before** its tiles, since removing the row removes the token every tile write is fenced on, so a build still running is rejected from that moment rather than writing into the gap between the two statements.
