...
None of that governs the **streamless** Strava activity, which never reaches `importFitnessFiles`: an activity with no positive duration and no streams gets `getOrCreateStravaFallbackNote` instead, which has always stamped the post at import time on every caller (`createNote` defaults to now) and mints its URI tail as a deterministic sha256 of the Strava activity id rather than a `publicId`, so that a re-import finds the same status instead of duplicating it.
