...
_A build folds an activity only if its own cursor says it has not already._ The scan runs newest-first and the build records the last file it took, so anything sorting at or after that cursor is skipped. Asking where a file sits, rather than counting how many came before it, is what makes the fold idempotent: a page redelivered after a crash, an offset that shifted because an activity was uploaded between two passes, a checkpoint whose progress write was lost — each re-presents files the build has already seen, and a counter cannot tell any of them from honest progress. The cursor advances for every file scanned, GPS or not, so a treadmill session at the front of a history does not leave it stuck.

_A build is either fresh or continuing, and the caller says which._ Keeping a build's version — adding to its tiles instead of replacing them — is only safe for a pass that carries on from where that build left off, so only a continuation may resume. A run starting from the beginning that inherited an abandoned `generating` row would otherwise fold the whole history a second time into that row's own version, which the completion sweep could then never clear. A pass that can cover neither the whole history nor the build's cursor hands the build back rather than sitting on it: its claim has already stamped a fresh heartbeat, and a live-looking build with no writer blocks every other claimant until the staleness window lapses.
...
Tiles and the progress describing them land in **one** guarded statement, so there is no window where the tiles are on disk and the cursor still points before them. The region row's checkpoint follows, which leaves the pyramid at or ahead of it and never behind — the harmless direction, since anything the region row hands back a second time is recognised by the cursor and skipped. The whole of it is isolated from the legacy blob: a tile-store failure abandons the build, records why on the pyramid row, and lets the run finish the heatmap the user can actually see.
