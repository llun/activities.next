...
- **Trimming applies to what this server renders, not to the uploaded file itself.** `GET /api/v1/fitness-files/:id` serves the original `.fit`/`.gpx`/`.tcx` bytes, and it answers any request — signed in or not — whenever the attached status is public or unlisted; the activity detail page links it as the source-file download for every viewer. That file still contains the full track, including the ends the map and route data trim. So a privacy location protects the rendered route on a public activity, not the coordinates of an activity that is public in the first place. Keep activities whose source file you do not want distributed at followers-only or private visibility.
