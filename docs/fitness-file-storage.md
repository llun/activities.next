...
- Privacy locations trim the head and tail of a route — not its middle — from route maps, route-data responses, and route heatmaps. A route that re-enters a zone part-way through is exposed there; see the Processing Pipeline above for why. A route whose exposed window collapses is not rendered at all. Route heatmaps annotate each activity separately, before pooling them, so every activity's own ends are trimmed rather than only the first and last of the pool.
