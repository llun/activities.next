...
- **The uploaded file is owner-only, because trimming cannot apply to bytes the server merely stores.** `GET /api/v1/fitness-files/:id` serves the original `.fit`/`.gpx`/`.tcx`, which still contains the full track including the ends the map and route data trim — so serving it to a viewer handed back exactly what every other surface had just hidden. It used to answer any request whenever the attached status was public or unlisted, and the activity detail page linked it for every viewer; now both are restricted to the owner, and the source-file row is hidden from everyone else (the file **name** is a disclosure in its own right). Nothing federated is affected: the URL is stripped from every outbound payload and never appeared in an email.
