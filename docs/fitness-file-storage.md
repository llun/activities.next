...
Activity route maps can also receive AI-generated accessibility alt-text descriptions describing the route geometry when `ACTIVITIES_ALT_TEXT_*` is configured at instance level and the actor has enabled **Generate AI route description** under Fitness Settings (`/fitness/privacy`, off by default). Route descriptions describe the shape and key features of the route directly without meta-preambles, and failures degrade gracefully without blocking activity processing or map regeneration. Existing activities can receive descriptions by running **Regenerate maps for old statuses** from the same page.
...
4. If visible GPS coordinates remain, a route map is rendered to a PNG buffer and stored as media — re-encoded to WebP, the default for every stored image — then inserted as the first status attachment named `Activity route map`. If automated alt text generation is configured (`ACTIVITIES_ALT_TEXT_*`) and enabled by the actor in Fitness Settings, an LLM generates a concise description of the route shown on the map for the media record's alt text description; if unconfigured, disabled, or generation fails, the media is stored without a description, ensuring activity processing is never blocked. When the import is an unattended one that emails the owner, a JPEG copy of the same map is stored for that email and recorded in `mapImageEmailPath`.
