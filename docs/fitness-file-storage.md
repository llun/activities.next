...
The two pyramid tables exist and have database accessors, but nothing writes to or reads from them yet: `fitness_route_heatmaps` still serves every heatmap. The tiler, the generation job, the serving API and the client's zoom-aware fetching land in later changes.
