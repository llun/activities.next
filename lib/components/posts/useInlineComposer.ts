...
export interface InlineComposerControls {
  ...
  openReply: (status: Status) => void
  openQuote: (status: Status) => void
  openEdit: (status: EditableStatus) => void
  close: () => void
}
 ...
 * straight to the post action row and render `InlineStatusComposer` under the
 * post whose id matches `active.status.id`.
 */
export const useInlineComposer = (): InlineComposerControls => {
  ...
  const openReply = useCallback((status: Status) => {
    setActive({ status: status as StatusNote | StatusPoll, mode: 'reply' })
  }, [])
  const openQuote = useCallback((status: Status) => {
    setActive({ status: status as StatusNote | StatusPoll, mode: 'quote' })
  }, [])
  const openEdit = useCallback((status: EditableStatus) => {
    setActive({ status, mode: 'edit' })
  }, [])
