...
describe('importStravaArchiveJob', () => {
  ...
  describe('gear attribution', () => {
    // The archive's `Activity Gear` column and its `bikes.csv`/`shoes.csv` are
    // deliberately not read. Attribution comes from the athlete's shed
    // instead, applied by `processFitnessFileJob` from the gear whose
    // `defaultSports` claims the parsed sport — the mapping the Strava
    // settings page edits. Creating gear from archive names meant a decade-old
    // export silently populated the shed with every bike ever owned.
    it('never creates gear from the archive', async () => {
      mockArchiveReaderOpen.mockResolvedValue({
        close: vi.fn(),
        hasEntry: vi.fn().mockReturnValue(true),
        getActivities: vi.fn().mockResolvedValue([
      ...
      // The activity itself still imported.
      expect(mockSaveFitnessFile).toHaveBeenCalledTimes(1)
      expect(database.updateStravaArchiveImport).toHaveBeenCalledWith(
