...
describe('renderEmail', () => {
  describe('reply marker', () => {
    it('is absent unless the message is repliable', () => {
      const { text, html } = render()
      expect(text).not.toContain(REPLY_SENTINEL)
      expect(html).not.toContain(REPLY_SENTINEL)
    })

    it('is the first line of the text part', () => {
      const { text } = render({ replyMarker: REPLY_SENTINEL })
      expect(text.split('\n')[0]).toBe(REPLY_SENTINEL)
    })

    // Order is the whole point: the inbound parser keeps everything ABOVE the
    // marker, so anything rendered ahead of it — the preheader, the header
    // wordmark — would be swept into the published reply.
    it('precedes the preheader and the header in the html', () => {
      const { html } = render({
        preheader: 'Choose a new password.',
        replyMarker: REPLY_SENTINEL
      })

      expect(html.indexOf(REPLY_SENTINEL)).toBeGreaterThan(-1)
      expect(html.indexOf(REPLY_SENTINEL)).toBeLessThan(
        html.indexOf('Choose a new password.')
      )
      expect(html.indexOf(REPLY_SENTINEL)).toBeLessThan(
        html.indexOf('Activities')
      )
    })

    // The end-to-end contract: a reply quoting this email extracts only what
    // the sender typed above it.
    it('lets the parser recover the sender text from the text part', () => {
      const { text } = render({ replyMarker: REPLY_SENTINEL })

      expect(extractReplyText({ text: `My answer.\n\n${text}` })).toBe(
        'My answer.'
      )
    })
