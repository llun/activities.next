...
describe('(timeline)/(home) page GIFV enrichment regression', () => {
  ...
  it('enriches unclassified video attachment to gifv on the very first home timeline render', async () => {
    ...
    const VIDEO_URL =
      'https://files.mastodon.social/media_attachments/files/117/262/540/412/068/294/original/gif-video.mp4'
    const PREVIEW_URL =
      'https://files.mastodon.social/media_attachments/files/117/262/540/412/068/294/small/gif-preview.png'

    // Seed post with no playbackType
    ...
    // Render the home page directly — NO prior detail page visit, NO backfill run!
    const node = await Page()
    const timelineElement = findElementByType(node, MainPageTimeline)

    expect(timelineElement).not.toBeNull()
    const timelineStatuses = timelineElement?.props.statuses as Array<{
      id: string
      attachments: Array<{ playbackType?: string; thumbnailUrl?: string }>
    }>

    const cheeaunPost = timelineStatuses.find((s) => s.id === note.id)
    expect(cheeaunPost).toBeDefined()
    expect(cheeaunPost?.attachments[0]?.playbackType).toBe('gifv')
    expect(cheeaunPost?.attachments[0]?.thumbnailUrl).toBe(PREVIEW_URL)
  })
})
