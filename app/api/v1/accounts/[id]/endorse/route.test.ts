...
const createRequest = (targetId: string) =>
  new NextRequest(
    `https://llun.test/api/v1/accounts/${urlToId(targetId)}/endorse`,
    { method: 'POST', headers: { origin: 'https://llun.test' } }
  )

describe('POST /api/v1/accounts/:id/endorse', () => {
  ...
  it('endorses a followed account and reflects endorsed=true', async () => {
  ...
  it('returns 422 when the current actor does not follow the target', async () => {
