...
COPY --from=build --chown=app:app /opt/activities.next/public /opt/activities.next/
COPY --from=build --chown=app:app /opt/activities.next/.next/static /opt/activities.next/
